...
2. 创建一个`lit-localize.json`配置文件<!-- ([details](#config-file))-->
3. 运行`lit-localize`提取以生成XLIFF文件
<!-- ([details](#extracting-messages)). -->
4. 编辑生成的XLIFF文件以添加`<target>`翻译标记<!-- ([details](#translation-with-xliff)). -->
5. 运行`lit-locationbuild`以输出字符串和模板的本地化版本 <!-- templates ([details](#output-modes)). -->
...
**不确认使用何种模式?** 从运行时模式开始.切换模式很容易,因为核心`msg`API是相同的.
